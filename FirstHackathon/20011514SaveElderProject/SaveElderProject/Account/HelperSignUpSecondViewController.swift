...
//        let elderCode = uid.enumerated().filter { $0.offset < 6 }.reduce("") { $0 + String($1.element) }
//        let elderCode = uid.enumerated().filter { (index, value) in index < 6 }.reduce(""){ (first, second) in String(first) + String(second.element) }
